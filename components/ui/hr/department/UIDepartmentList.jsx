...
export default function UIDepartmentList({ header, data = [], error = "" }) {
  ...
  const columns = useMemo(() => {
    const baseColumns = [
      { name: "ลำดับ", uid: "departmentId" },
      { name: "ฝ่าย", uid: "divisionName" },
      { name: "แผนก", uid: "departmentName" },
      { name: "สถานะการใช้งาน", uid: "departmentStatus" },
    ];

    if (canManage) {
      baseColumns.push(
        { name: "สร้างโดย", uid: "createdBy" },
        { name: "สร้างเมื่อวันที่", uid: "departmentCreateAt" },
        { name: "แก้ไขโดย", uid: "updatedBy" },
        { name: "แก้ไขเมื่อวันที่", uid: "departmentUpdateAt" },
  ...
  const handleAction = useCallback(
    (action, item) => {
      if (action === "edit") {
        router.push(`/department/${item.departmentId}`);
      }
    },
    [router]
  );
  ...
  const renderCell = useCallback(
    (item, idx, colKey) => {
      switch (colKey) {
        ...
        case "divisionName":
          return item.DepartmentDivisionId?.divisionName || "-";
        case "createdBy":
          return item.DepartmentCreateBy
            ? `${item.DepartmentCreateBy.empFirstNameTH} ${item.DepartmentCreateBy.empLastNameTH}`
            : "-";
        case "updatedBy":
          return item.DepartmentUpdateBy
            ? `${item.DepartmentUpdateBy.empFirstNameTH} ${item.DepartmentUpdateBy.empLastNameTH}`
            : "-";
        case "actions":
